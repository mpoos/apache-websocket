        ...
        while (tpd->active && tpd->tcpsocket) {
            ...
            if (num<=0) {
                ...
                if (rv == APR_SUCCESS) {
                    ...
                    apr_sleep(10000);      /* this should not happen */
                }
                                            ...
                                            const size_t buffer_size)
{
    ...
    if (tpd && tpd->tcpsocket) {
        ...
        if (tpd->base64) {
            ...
            len = apr_base64_decode_binary(towrite, (const char *)ztbuf);
            free(ztbuf);
            if (len <= 0) {
                free(towrite);
                towrite = NULL;
        ...
        while (l>0) {
            ...
            if (num>0) {
                apr_size_t lw = l;
                rv = apr_socket_send(tpd->tcpsocket, (const char *)p, &lw);
